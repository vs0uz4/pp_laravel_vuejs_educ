@extends('layouts.app')

@section('content')
    <div class="container">
        <div class="row">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3><span class="glyphicon glyphicon-list-alt"></span> Subjects</h3>
                </div>
                <div class="panel-body">
                    {!! Button::success('New Subject')->prependIcon(Icon::plusSign())->asLinkTo(route('admin.subjects.create')) !!}
                </div>

                {!!
                    Table::withContents($subjects->items())
                        ->hover()
                        ->withClassOnCellsInColumn(['ID','Actions'], 'text-center')
                        ->callback('Actions', function ($field, $row){
                            (!$_REQUEST) ? $page = 1 : $page = $_REQUEST['page'];
                            $linkEdit = route('admin.subjects.edit', ['subject' => $row->id, 'page' => $page]);
                            $linkShow = route('admin.subjects.show', ['subject' => $row->id, 'page' => $page]);

                            return Button::link('Edit')->prependIcon(Icon::edit())->asLinkTo($linkEdit). '|' . Button::link('View')->prependIcon(Icon::eyeOpen())->asLinkTo($linkShow);
                        })
                !!}
            </div>
            <div class="row text-center">
                {!! $subjects->links() !!}
            </div>
        </div>
    </div>
@endsection